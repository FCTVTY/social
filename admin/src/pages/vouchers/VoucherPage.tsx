...
export const VoucherPage: FC = () => {
  ...
  const fetchData = async () => {
    try {
      const vouchersResponse = await axios.get<Ads[]>(
        `${getApiDomain()}/v1/data/get`,
      );
      ...
      <main className="container mx-auto px-4 py-6">
        ...
        <ul role="list" className="divide-y divide-gray-200 mt-4">
          {vouchers.map((voucher) => (
            <li
              key={voucher._id}
              className="relative flex items-center space-x-4 p-4 hover:bg-gray-100 rounded-lg"
            >
              <img
                src={voucher.logo}
                alt={voucher.name}
              ...
              <div className="flex-auto">
                ...
                <p className="mt-2 text-sm text-gray-600">{voucher.ad}</p>
              </div>
              <a
                href={`/ad/edit/${voucher._id}`}
