...
export default function EventPage({ host, channel, post }: HomeProps) {
    ...
    const handleLikeClick = async () => {
        ...
        if (userHasLiked) {
            ...
            // Add the like
            setGoing(true);
            updatedLikes = [...postLikes, { _id: new Date().getTime().toString(), postId: ppost?._id ?? "", userId: profile?.supertokensId ?? "" }];
        }
      ...
      <>
          {ppost && (
            <div>
                            ...
                            <div className="mt-6 sm:flex sm:min-w-0 sm:flex-1 sm:items-center sm:justify-end sm:space-x-6 sm:pb-1">
                                ...
                                <div className="mt-6 flex flex-col justify-stretch space-y-3 sm:flex-row sm:space-x-4 sm:space-y-0">
                                    {!userHasLiked ? (
                                      ...
                                      >
                                          <CheckCircleIcon className="-ml-0.5 mr-1.5 h-5 w-5 text-white" aria-hidden="true" />
                                          <span>Going</span>
                                      </div>
                                    )}
                        ...
                        <li className="col-span-1 flex flex-col divide-y divide-gray-200 rounded-lg bg-white shadow max-w-4xl">
                            <div className="flex flex-1 flex-col p-3">
                                <dl className="mt-1 flex flex-grow flex-col justify-between">
                                    ...
                                    {ppost.article ? (
                                      <dd className="text-sm text-gray-500 my-3"
                ...
                <Comment onSubmit={fetchDetails} post={post} supertokensId={profile?.supertokensId}/>
