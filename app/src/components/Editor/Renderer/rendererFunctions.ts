...
const customEntityTransform = (
  ...
  if (entity.type === "IMG")
    return `<div className='relative w-full aspect-video RTEImageContainer grid place-items-center bg-red-200'>
        <img className='w-auto max-w-full' loading='lazy' src=${entity.data.url} alt=${text}/>
    </div>`;
  // LINK
  if (entity.type === "LINK")
    return `<a title=${entity.data.url} href="${entity.data.url}" target="_blank" rel="noopener noreferrer">${text}</a>`;
...
const processHTMLString = (text: string): string => {
  ...
  const processedText = text.replace(
    /---/g,
    "<div class='w-full h-[0.1rem] bg-foreground/10'></div>",
  );
