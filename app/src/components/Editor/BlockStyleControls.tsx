...
const HeadersMenu: React.FC<{
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger>
        <p
          className={`select-text rounded-md aspect-square h-10 grid place-items-center p-2  bg-foreground/10 opacity-50 hover:opacity-70`}
        >
          T
        </p>
        <DropdownMenuContent>
          {HEADERS.map((header) => (
            <DropdownMenuItem
              className={`${header.style === blockType ? "opacity-100 bg-foreground/10" : "opacity-70"}`}
              onClick={() => {
                onToggle(header.style);
              }}
              key={header.label}
            >
              {header.label}
